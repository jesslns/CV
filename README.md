...
| :---- | :---- | :---- | :---- |
|AsanaApp<br>([Github](https://github.com/nomats/asanaApp))|A yoga pose app that can check if the user has the correct pose. <br> The app can capture a user's pose through a webcam and send data to PoseNet(a machine learning model that allows for real-time human pose estimation). PoseNet then returns spatial data from 17 pose key points. Those data will be processed by a model that we implemented and check if relationship between keypoints can fulfill the criteria for a certain yoga pose.|2 weeks|JavaScript, REACT, Testing with Jest|
