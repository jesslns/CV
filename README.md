...
| :---- | :---- | :---- | :---- |
|AsanaApp<br>([Github](https://github.com/nomats/asanaApp))|A yoga pose app that can check if the user has the correct pose. <br> The app can capture a user's pose through a webcam and sent data to PoseNet. PoseNet then return spatial data from 17 pose key points. Those data will be processed by a model that we implemented and check if relationship between keypoints can fulfill the criteria for a certain yoga pose.|2 weeks|JavaScript, REACT, Testing with Jest|
